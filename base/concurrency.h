...
//
#ifndef BASE_CONCURRENCY_H_
#define BASE_CONCURRENCY_H_
...
class BoundedQueue {
 public:
  BoundedQueue(uint32_t size)
    ...
    void* ring;
    if (posix_memalign(&ring, EV_CACHELINE_SIZE, size * sizeof(Slot)))
      throw std::bad_alloc();
 ...
 private:
  struct alignas(EV_CACHELINE_SIZE) Slot : public WaitType {
    ValueType value;
  };
  ...
  alignas(EV_CACHELINE_SIZE) std::atomic<uint64_t> head_;
  alignas(EV_CACHELINE_SIZE) std::atomic<uint64_t> tail_;
};
